...
const SignupPage: React.FC<SignupPageProps> = ({ onSignup, onNavigateToLogin }) => {
  ...
  const handleSubmit = useCallback((e: React.FormEvent) => {
    e.preventDefault();
    const mockUser: User = { name, email, picture: '' };
    onSignup(mockUser);
  }, [name, email, onSignup]);
  ...
  const handleCredentialResponse = useCallback((response: any) => {
    ...
    if (userObject) {
      console.log("Signed up user:", userObject);
      const user: User = {
  ...
  return (
    <div className="w-full max-w-md p-8 space-y-6">
        <div className="flex flex-col items-center space-y-2">
            ...
            <h2 className="text-xl font-semibold text-text-secondary">Create Your Account</h2>
        </div>
        <div className="space-y-4">
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                <label htmlFor="email-signup" className="text-sm font-medium text-text-secondary">Email</label>
                <input
                    id="email-signup"
                    type="email"
                    value={email}
                ...
                <div className="space-y-2">
                <label htmlFor="password-signup" className="text-sm font-medium text-text-secondary">Password</label>
                <input
                    id="password-signup"
                    type="password"
                    value={password}
        ...
        <div className="text-center">
            ...
            <button onClick={onNavigateToLogin} className="font-medium text-primary hover:underline focus:outline-none">
                Login
            </button>
            </p>
        </div>
